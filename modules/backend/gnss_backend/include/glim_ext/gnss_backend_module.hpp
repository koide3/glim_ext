 ...
 */
class GNSSBackend : public ExtensionModuleBase {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  GNSSBackend() {
    spdlog::info("initializing GNSS backend");
    auto global_config = glim::GlobalConfig::instance();
    std::string config_path;
    if (global_config->param<std::string>("global", "config_gnss_backend")) {
      config_path = global_config->get_config_path("config_gnss_backend");
    } else {
      config_path = glim::GlobalConfigExt::get_config_path("config_gnss");
    }

    spdlog::info("gnss_backend_config_path={}", config_path);
  ...
  void backend_task() {
    spdlog::info("starting GNSS backend thread");
    ...
    while (!kill_switch) {
      ...
      while (!utm_queue.empty() && !submap_queue.empty() && submap_queue.front()->frames.front()->stamp > utm_queue.front()[0] &&
        ...
        if (right == utm_queue.end() || (right + 1) == utm_queue.end()) {
          spdlog::warn("invalid condition in GNSS backend module!!");
          break;
        }
